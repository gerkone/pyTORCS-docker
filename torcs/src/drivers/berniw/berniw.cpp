...
// socket variables - gerk
static int UDP_TIMEOUT = UDP_DEFAULT_TIMEOUT;
static int listenSocket[BOTS];
socklen_t clientAddressLength[BOTS];
struct sockaddr_in clientAddress[BOTS], serverAddress[BOTS];
...
static void shutdown(int index) {
	...
	// send shutdown message to client - gerk
...
static void initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation * situation)
{
	...
	*carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
	...
	static void newRace(int index, tCarElt* car, tSituation *situation)
	{
		...
		// the rest of the function is the UDP socket setup - gerk
		char line[UDP_MSGLEN];
		...
		serverAddress[index].sin_port = htons(UDP_LISTEN_PORT + (index - 1));
		...
		if (bind(listenSocket[index],
			...
			std::cout << "Waiting for request on port " << UDP_LISTEN_PORT + (index - 1) << "\n";
			...
			while (!identified)	{
				...
				if (recvfrom(listenSocket[index], line, UDP_MSGLEN, 0,(struct sockaddr *) &clientAddress[index], 	&clientAddressLength[index]) < 0)	{
		...
		/* controls the car, called each step */
		static void drive(int index, tCarElt* car, tSituation *situation)
		{
			tdble angle;
			tdble brake;
			...
			if (!mpf->getPitStop() && (car->_remainingLaps-car->_lapsBehindLeader) > 0 && (car->_dammage > myc->MAXDAMMAGE ||
				(car->priv.fuel < 1.5*myc->fuelperlap &&
					...
					float tel_angle =  RtTrackSideTgAngleL(&(car->_trkPos)) - car->_yaw;
					NORM_PI_PI(tel_angle); // normalize the angle between -PI and + PI
					...
					// update time
					// count time with chrono because the 50Hz RT clock is unstable
					...
					float combinedThrottle = car->_accelCmd - car->_brakeCmd;
					// action
					telemetry =  SimpleParser::stringify("steer", car->_steerCmd);
					telemetry += SimpleParser::stringify("throttle", combinedThrottle);

					// state
					telemetry +=  SimpleParser::stringify("damage", 0);
					telemetry +=  SimpleParser::stringify("angle", tel_angle);
					telemetry += SimpleParser::stringify("distFromStart", car->race.distFromStartLine);
					telemetry += SimpleParser::stringify("distRaced", car->race.distRaced);
					telemetry += SimpleParser::stringify("totalTime", totalTime);
					telemetry += SimpleParser::stringify("trackLen", curTrack->length);
					telemetry += SimpleParser::stringify("rpm", car->_enginerpm*10);
					telemetry += SimpleParser::stringify("speedX", float(car->_speed_x  * 3.6));
					telemetry += SimpleParser::stringify("speedY", float(car->_speed_y  * 3.6));
					telemetry += SimpleParser::stringify("speedZ", float(car->_speed_z  * 3.6));
					telemetry += SimpleParser::stringify("track", trackSensorOut, 19);
					telemetry += SimpleParser::stringify("trackPos", dist_to_middle);
					telemetry += SimpleParser::stringify("wheelSpinVel", wheelSpinVel, 4);

					char line[UDP_MSGLEN];
					struct timeval timeVal;
					fd_set readSet;
					sprintf(line, "%s", telemetry.c_str());

					// Sending the car state to the client
					if (sendto(listenSocket[index], line, strlen(line) + 1, 0, (struct sockaddr *) &clientAddress[index], sizeof(clientAddress[index])) < 0) {
						std::cerr << "Error: cannot send telemetry";
					}
