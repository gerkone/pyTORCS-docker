...
static unsigned char* tmpRGBimg = (unsigned char*)malloc( 3 * IMG_WIDTH * IMG_HEIGHT * sizeof(unsigned char) );
...
*/
	...
	if(max != 0){
		...
		*/
		...
		double avg,r,g,b,min,max,s,delta;
		for (int pixel=0; pixel < 3*IMG_WIDTH*IMG_HEIGHT; pixel++)
		{
...
ReStart(void)
{
    ...
    if( getVision() ){
      ...
      if( IMG_WIDTH > 0 ) {
        ReInfo->vision->sw = ReInfo->vision->vw = IMG_WIDTH;
				ReInfo->vision->sh = ReInfo->vision->vh = IMG_HEIGHT;
			}
      ...
      memset(tmpRGBimg, 1, 3 * IMG_WIDTH * IMG_HEIGHT * sizeof(unsigned char));
