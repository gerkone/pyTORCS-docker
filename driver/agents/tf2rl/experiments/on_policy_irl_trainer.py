...
class OnPolicyIRLTrainer(Trainer):
    def __init__(self, policy, env, args, irl, expert_obs, expert_next_obs, expert_act, test_env):
    ...
    def __call__(self):
        ...
        self.replay_buffer = get_replay_buffer(
            ...
            self._use_nstep_rb, self._n_step, irl = True)
        ...
        while total_steps < self._max_steps:
            ...
            for _ in range(self._policy.horizon):
                ...
                next_obs, real_reward, done = self._env.step(env_act)
                next_obs = unpack_state(next_obs)

                # get reward from discriminator
                reward = np.squeeze(self._irl.inference(obs, act, next_obs))
            ...
            with tf.summary.record_if(total_steps % self._save_summary_interval == 0):
                ...
                for _ in range(self._irl.n_training):
                    samples = self.replay_buffer.sample(self._irl.batch_size)
    ...
    def finish_horizon(self, last_val=0):
        ...
        self.replay_buffer.add(
            obs=samples["obs"], next_obs=samples["next_obs"], act=samples["act"], done=samples["done"],
            ret=rets, rew = samples["rew"], adv=advs, logp=np.squeeze(samples["logp"]))
